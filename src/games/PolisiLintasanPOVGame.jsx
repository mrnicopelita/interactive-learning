...
const SPEED_LEVELS = [
  { level: 1, emoji: '🐢', factor: 2.2 },
  { level: 2, emoji: '🚔', factor: 3.0 },
  { level: 3, emoji: '🚀', factor: 3.9 },
]
...
const FEMALE_HINTS = [
  ...
  /michelle|sonia|serena|millie|hayley|linda|julie/i,
...
function speak(text, opts = {}) {
  ...
  try {
    ...
    u.lang = 'en-US'; u.volume = 1; u.rate = opts.rate ?? 1.28; u.pitch = opts.pitch ?? 1.4
...
const WIN_Z = GATES[MAX_TURNS - 1] + GAP * 1.45 + 2
...
export default function PolisiLintasanPOVGame({ onExit }) {
  ...
  const changeSpeed = useCallback((lvl) => { speedRef.current = lvl; setSpeedLevel(lvl) }, [])

  const registerOb = useCallback((i) => (el) => {
    if (el) obRefs.current.set(i, el); else obRefs.current.delete(i)
  }, [])
  const registerStrip = useCallback((i) => (el) => {
    if (el) stripRefs.current.set(i, el); else stripRefs.current.delete(i)
  }, [])
  ...
  useEffect(() => {
    ...
    const tick = (ts) => {
      ...
      if (running) {
        ...
        worldZRef.current = Math.min(WIN_Z, worldZRef.current + sp * dt)
      }
      ...
      for (let i = 0; i < 9; i++) {
        ...
        if (!el) continue
        const d = mod((i * (D_MAX - D_MIN)) / 9 - z - D_MIN, D_MAX - D_MIN) + D_MIN
      ...
      for (let i = 0; i < MAX_TURNS; i++) {
        ...
        if (el) {
          ...
          if (visible) {
            const s = SPD / d
            const ti = i % 4
            const sz = Math.max(100, h * 0.5) * s
            const aspect = [1, 0.8, 0.6, 0.6][ti]
            const x = w / 2
            const y = yOf(d)
        ...
        if (running && !passedRef.current.has(i)) {
      ...
      if (nearIdx !== cueIdxRef.current) {
        ...
        if (siapRef.current !== next) { siapRef.current = next; setSiap(next) }
      }
  ...
  function restart() {
    ...
    setTurns(0); setSiap(false); setTurnFx(0); setLandFx(0)
    setSirenActive(false); setPaused(false); setPhase('run')
  }
  ...
  return (
    <div className="flex h-dvh w-full touch-manipulation select-none flex-col overflow-hidden bg-gradient-to-b from-sky-300 via-sky-100 to-green-200">
      <header className="z-30 flex w-full shrink-0 flex-wrap items-center justify-between gap-2 px-3 py-2 sm:px-5 sm:py-3">
        ...
        <button type="button" onClick={restart} className="rounded-full bg-amber-500/90 px-4 py-2 text-sm font-extrabold text-white shadow-[0_4px_0_rgba(180,83,9,0.8)] transition hover:scale-105 hover:bg-amber-500 sm:px-5 sm:text-base">🔁 Ulangi</button>
      </header>
      ...
      <div className="z-30 flex w-full shrink-0 flex-wrap items-center justify-center gap-x-5 gap-y-1 px-2 py-1 sm:py-2">
        <div className="flex flex-col items-center gap-1">
          ...
          <span className={`text-xl font-black text-sky-800 sm:text-2xl ${turns > 0 ? 'animate-pop-in' : ''}`}>{turns} / {MAX_TURNS}</span>
        </div>
        <div className="flex flex-col items-center gap-1">
          <span className="text-xs font-extrabold tracking-wide text-amber-800 uppercase sm:text-sm">Kecepatan</span>
          <div className="flex items-center gap-1.5 sm:gap-2">
            {SPEED_LEVELS.map((sl) => (
              <button key={sl.level} type="button" onClick={() => changeSpeed(sl.level)}
                className={`flex items-center gap-1 rounded-full px-3 py-1.5 text-sm font-black shadow transition sm:px-4 sm:py-2 sm:text-base ${speedLevel === sl.level ? 'scale-105 bg-amber-400 text-white shadow-[0_3px_0_#b45309] ring-2 ring-amber-300' : 'bg-white/90 text-amber-700 hover:scale-105'}`}>
                <span aria-hidden="true">{sl.emoji}</span>{sl.level}
              </button>
            ))}
          </div>
        </div>
        <button type="button" onClick={togglePause} className="rounded-full bg-white/90 px-4 py-1.5 text-sm font-black text-slate-600 shadow transition hover:scale-105 sm:px-5 sm:py-2 sm:text-base">{paused ? '▶️ Lanjut' : '⏸️ Istirahat'}</button>
      </div>
      ...
      <div ref={stageRef} className="relative min-h-0 flex-1 overflow-hidden">
        <div ref={sceneRef} className="absolute inset-0 will-change-transform">
          ...
          <span aria-hidden="true" className="animate-floaty absolute right-[6%] top-[6%] text-5xl opacity-90 drop-shadow-[0_0_16px_rgba(250,204,21,0.8)] sm:text-7xl">☀️</span>
          {[{ top: '16%', d: '26s', delay: '0s', s: 'text-4xl sm:text-6xl' }, { top: '34%', d: '34s', delay: '-12s', s: 'text-5xl sm:text-7xl' }, { top: '52%', d: '29s', delay: '-24s', s: 'text-3xl sm:text-5xl' }, { top: '10%', d: '38s', delay: '-6s', s: 'text-4xl sm:text-6xl' }].map((c, i) => (
            <span key={i} aria-hidden="true" className={`animate-cloud-drift pointer-events-none absolute left-0 select-none opacity-60 ${c.s}`} style={{ top: c.top, animationDuration: c.d, animationDelay: c.delay }}>☁️</span>
          ))}

          {/* HORIZON DISTANT MOUNTAINS */}
          <svg className="absolute bottom-0 left-0 h-24 w-full opacity-70 sm:h-32" viewBox="0 0 1440 160" preserveAspectRatio="none" aria-hidden="true" style={{ top: `${horizon - 60}px` }}>
            <path d="M0 40 C 180 10, 300 80, 480 55 C 650 32, 760 90, 960 60 C 1120 38, 1240 78, 1440 45 L 1440 160 L 0 160 Z" fill="#16a34a" />
            <path d="M0 110 C 220 70, 420 140, 680 100 C 900 68, 1120 130, 1440 92 L 1440 160 L 0 160 Z" fill="#15803d" />
          </svg>

          {/* GROUND — green grass base */}
          <div className="absolute left-0 right-0" style={{ top: `${horizon}px`, bottom: 0 }}>
            <div className="h-full w-full" style={{ background: 'linear-gradient(to bottom, #22c55e 0%, #16a34a 20%, #15803d 50%, #166534 100%)' }} />
          </div>

          {/* SAND STRIPS on left and right edges */}
          <div className="absolute bottom-0 left-0 top-0" style={{ width: '14%', background: 'linear-gradient(to right, #d4a052, #c29348 60%, transparent)' }} />
          <div className="absolute bottom-0 right-0 top-0" style={{ width: '14%', background: 'linear-gradient(to left, #d4a052, #c29348 60%, transparent)' }} />

          {/* ROAD — gray asphalt in the center */}
          <div className="absolute bottom-0 top-0" style={{ left: '30%', right: '30%', background: 'linear-gradient(to bottom, #4b5563 0%, #374151 25%, #1f2937 100%)' }} />

          {/* ROAD EDGE LINES — white solid on both sides */}
          <div className="absolute bottom-0 top-0" style={{ left: '30%', width: '3px', background: '#fff', opacity: 0.7 }} />
          <div className="absolute bottom-0 top-0" style={{ right: '30%', width: '3px', background: '#fff', opacity: 0.7 }} />

          {/* CENTER DASHED WHITE LINE */}
          {Array.from({ length: 9 }, (_, i) => (
            <span key={`strip-${i}`} ref={registerStrip(i)} className="absolute" style={{ left: 'calc(50% - 1.5px)', width: '3px', background: '#fff', opacity: 0.85 }} aria-hidden="true" />
          ))}

          {/* TURN SIGNS */}
          {Array.from({ length: MAX_TURNS }, (_, i) => (
            <span key={`turn-${i}`} ref={registerOb(i)} className="absolute" style={{ display: 'none' }} aria-hidden="true">
              <TurnSign dir={TURN_DIRS[i % TURN_DIRS.length]} />
            </span>
          ))}

          {/* GOLD BADGE at the end */}
          <span ref={goldRef} className="absolute" style={{ display: 'none' }} aria-hidden="true">
            <svg viewBox="0 0 240 300" className="h-auto w-full drop-shadow-[0_16px_24px_rgba(146,64,14,0.45)]">
        ...
        <div key={landFx} className="animate-pov-ring pointer-events-none absolute z-30" style={{ left: '50%', top: '88%', width: '130%', height: '80%' }} aria-hidden="true">
        ...
        {turnFx > 0 && phase === 'run' && (
          <div key={`splash-${turnFx}`} className="animate-pov-splash pointer-events-none absolute left-1/2 top-[38%] z-50 flex -translate-x-1/2 flex-col items-center">
            <span className="text-[clamp(3.5rem,12vw,8rem)] font-black leading-none text-white drop-shadow-[0_6px_0_rgba(2,6,23,0.3)]">MIRING!</span>
            <span className="text-[clamp(2.2rem,7.5vw,4.5rem)]" aria-hidden="true">🚔🔄</span>
          </div>
        )}
        ...
        {phase === 'start' && (
          <div className="absolute inset-0 z-[60] flex flex-col items-center justify-center gap-4 bg-[radial-gradient(circle,rgba(240,253,244,0.97)_0%,rgba(187,247,208,0.95)_100%)] px-4">
            <div className="animate-pop-in flex flex-col items-center text-center">
              <span className="text-[clamp(2.5rem,8vw,4rem)]" aria-hidden="true">🎥🚔</span>
              <h2 className="text-[clamp(1.6rem,5vw,2.6rem)] font-black text-emerald-900">GoPro di atas mobil polisi!</h2>
              <p className="max-w-xl text-[clamp(1rem,3vw,1.35rem)] font-bold text-emerald-800">Lihat jalan dari mata mobil polisi! Saat ada tikungan, <span className="text-amber-600">MIRING</span> ke kiri atau kanan! 🔄</p>
              <p className="max-w-xl text-sm font-bold text-emerald-700 sm:text-base">Berdiri tegak, lalu miringkan tubuhmu mengikuti tikungan!</p>
            </div>
            <button type="button" onClick={restart} className="animate-pov-replay rounded-full bg-emerald-500 px-10 py-4 text-xl font-black text-white shadow-[0_6px_0_#047857] transition hover:scale-105 sm:text-2xl">🚔 Mulai Patroli!</button>
          </div>
        )}
        ...
        {phase === 'win' && (
          <div className="absolute inset-0 z-[60] flex flex-col items-center justify-center gap-2 bg-[radial-gradient(circle,rgba(255,244,214,0.97)_0%,rgba(253,230,138,0.93)_45%,rgba(245,197,24,0.88)_100%)] px-4">
            <div className="animate-hore-bounce flex items-center gap-2 text-center">
              <span className="text-[clamp(2rem,7vw,4rem)] font-black text-amber-800 drop-shadow-[0_4px_0_rgba(146,64,14,0.35)]">HORE!</span>
              <span className="text-[clamp(1.6rem,5vw,3rem)]" aria-hidden="true">🚔</span>
            </div>
            <svg viewBox="0 0 240 300" className="h-auto w-[min(48vw,270px)] drop-shadow-[0_18px_30px_rgba(146,64,14,0.5)]">
              <circle cx="120" cy="130" r="90" fill="#f5c518" stroke="#b45309" strokeWidth="6" />
              <circle cx="120" cy="130" r="70" fill="#fbbf24" stroke="#b45309" strokeWidth="4" />
              <path d="M120 40 l18 36 40 5 -29 28 7 40 -36 -19 -36 19 7 -40 -29 -28 40 -5 Z" fill="#f59e0b" stroke="#b45309" strokeWidth="3" />
              <text x="120" y="140" textAnchor="middle" fill="#92400e" fontSize="28" fontWeight="bold" fontFamily="sans-serif">POLISI</text>
            ...
            <p className="animate-pop-in text-center text-[clamp(1rem,3vw,1.5rem)] font-extrabold text-amber-800">Kamu menemukan Lencana Emas! ⭐🎉</p>
            <p className="-mt-1 text-center text-sm font-extrabold text-amber-700 sm:text-base">Polisi melewati {turns} tikungan dengan selamat!</p>
            <p className="text-center text-sm font-extrabold text-amber-700 sm:text-base">Kamu juga ikut miring {turns} kali, kan? Keren! 💪</p>
            <div className="mt-2 flex flex-wrap items-center justify-center gap-3">
              <button type="button" onClick={restart} className="rounded-full bg-emerald-500 px-7 py-3 text-lg font-black text-white shadow-[0_5px_0_#047857] transition hover:scale-105 sm:text-xl">🚔 Patroli Lagi</button>
        ...
        {paused && phase === 'run' && (
          <div className="absolute inset-0 z-[55] flex flex-col items-center justify-center gap-3 bg-slate-900/40 backdrop-blur-[2px]">
            <div className="animate-pop-in flex flex-col items-center gap-4 rounded-3xl bg-white/95 px-8 py-6 text-center shadow-2xl">
              ...
              <button type="button" onClick={togglePause} className="rounded-full bg-emerald-500 px-8 py-3 text-lg font-black text-white shadow-[0_5px_0_#047857] transition hover:scale-105">▶️ Lanjut!</button>
            </div>
          </div>
        )}
      </div>
